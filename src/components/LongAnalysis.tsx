...
interface LongAnalysisProps {
  maxCoins?: number;
}

/**
 * VPVR POC 정보를 계산하는 헬퍼 함수 (Long용)
 */
function getVPVRLongInfo(
  currentPrice: number,
  ...
  // VPVR 점수 계산 (Long용)
  const score = calculateVPVRLongScore(currentPrice, vpvrPOC, atr) * 100;
...
export function LongAnalysis({ maxCoins: initialMaxCoins = 10 }: LongAnalysisProps) {
  ...
  const fetchData = async (coinCount?: number, days?: number) => {
    ...
    try {
      ...
      // 하락률 상위 코인 필터링 (Long: 하락한 코인이 반등할 가능성)
      ...
      for (let i = 0; i < validTickers.length; i++) {
        ...
        try {
          ...
          // Long 점수 계산 (ADX, ATR, 이동평균선, VWMA100, VPVR POC 추가)
          ...
          // 손절가/목표가 계산 (ATR 하이브리드 방식, Long 포지션)
          const stopLossInfo = calculateStopLoss(supportResistance, 'long', atr);
          ...
          scores.push({
            ...
            ticker,
            short_score: 0, // Long 분석에서는 사용하지 않음
      ...
      // Long 점수 기준 정렬 (타이밍 반영 전)
      scores.sort((a, b) => (b.long_score || 0) - (a.long_score || 0));
      ...
      try {
        ...
        if (marketDayHourPatternLocal && marketWeeklyPatternLocal) {
          ...
          // Long에 유리: 요일 하락 확률이 낮고(상승 확률이 높고), 시간대 평균이 양수
          const isFavorable = dayWinRate < 0.5 && hourAvgChange > 0;
          const dayNames = ['일요일', '월요일', '화요일', '수요일', '목요일', '금요일', '토요일'];
          const message = isFavorable
            ? `✅ 현재 시간대(${dayNames[currentDay]} ${currentHour}시)는 Long에 유리합니다 (요일 상승 확률: ${((1 - dayWinRate) * 100).toFixed(1)}%, 시간대 평균: ${hourAvgChange.toFixed(2)}%)`
            : `⚠️ 현재 시간대(${dayNames[currentDay]} ${currentHour}시)는 Long에 불리할 수 있습니다 (요일 상승 확률: ${((1 - dayWinRate) * 100).toFixed(1)}%, 시간대 평균: ${hourAvgChange.toFixed(2)}%)`;
        ...
        const timingScore = computeLongTimingScore(marketWeeklyPatternLocal, marketDayHourPatternLocal);
        for (const c of scores) {
          const base = c.long_score || 0; // base는 이미 0~80 범위
          c.long_score = Math.min(100, base + 0.20 * (timingScore * 100));
        }
        scores.sort((a, b) => (b.long_score || 0) - (a.long_score || 0));
      } catch (err) {
  ...
  return (
    <div className="short-analysis">
      <div className="header">
        <h1>Long 적합도 분석 (상위 {maxCoins}개 코인)</h1>
              ...
              {coinScores.map((coin, idx) => (
                <div key={coin.symbol} className="coin-list-item">
                  ...
                  <span className="coin-score-text">
                    점수: {(coin.long_score || 0).toFixed(2)}
                  </span>
          ...
          <div className="coin-score-card search-result-card">
            <div className="coin-score-header">
              ...
              <div className="short-score">
                Long 점수: <strong>{(searchResult.long_score || 0).toFixed(2)}/100</strong>
              </div>
              ...
              <div className="detail-section">
                ...
                <div className="detail-grid">
                  ...
                  {searchResult.vpvrPOC && (() => {
                    const vpvrInfo = getVPVRLongInfo(
      ...
      <div className="score-info">
        <h3>점수 계산 기준:</h3>
        <ul>
          <li>요일·시간대 타이밍: 20% (현재 요일·시간대가 Long에 유리할수록 가산)</li>
          <li>펀딩비: 20% (시간당 펀딩비 기준, 낮을수록 Long에 유리)</li>
          <li>ADX 트렌드: 18% (상승 트렌드이고 강할수록 유리, 횡보장 필터링)</li>
          <li>이동평균선: 18% (MA50, MA200, VWMA100 통합 - 거래량 가중 이동평균선 포함)</li>
          <li>RSI: 14% (낮을수록 과매도, Long에 유리, period 9 최적화)</li>
          <li>VPVR POC: 10% (현재가가 POC보다 높을수록 Long에 유리)</li>
        </ul>
      </div>
        ...
        <div className="weekly-pattern-section">
          ...
          <div className="weekly-pattern-summary">
            <div className="pattern-highlight">
              <span className="highlight-label">Long에 가장 유리한 요일:</span>
              <span className="highlight-value best">{marketWeeklyPattern.worstDay}</span>
              <span className="highlight-detail">
                {(() => {
                  ...
                  return `(상승 확률: ${((1 - (pattern?.winRate ?? 0.5)) * 100).toFixed(1)}%)`;
                })()}
              </span>
            </div>
            <div className="pattern-highlight">
              <span className="highlight-label">Long에 가장 불리한 요일:</span>
              <span className="highlight-value worst">{marketWeeklyPattern.bestDay}</span>
              <span className="highlight-detail">
                {(() => {
                  const dayMap: Record<string, DayKey> = {
                    '월요일': 'monday',
          ...
          <div className="weekly-pattern-grid">
            {[
              ...
              // 상승에 유리하면 초록색, 하락에 유리하면 빨간색 (Long 관점)
              const isFavorableForLong = pattern.winRate < 0.5; // 하락 확률이 50% 미만이면 Long 유리
              const colorClass = isFavorableForLong ? 'favorable-long' : 'favorable-short';
              const bestWorstClass = key === marketWeeklyPattern.worstDay.toLowerCase() ? 'best-day' : key === marketWeeklyPattern.bestDay.toLowerCase() ? 'worst-day' : '';
              ...
              <div key={key} className={`weekly-pattern-card ${colorClass} ${bestWorstClass}`}>
                ...
                <div className="pattern-stats">
                  <div className="stat-item">
                    <span className="stat-label">상승 확률:</span>
                    <span className={`stat-value ${pattern.winRate < 0.5 ? 'good' : 'bad'}`}>
                      {((1 - pattern.winRate) * 100).toFixed(1)}%
                    </span>
                  </div>
                  <div className="stat-item">
          ...
          return (
            <div key={coin.symbol} className={`coin-score-card ${isTop3 ? 'top-3-highlight' : ''}`}>
              <div className="coin-score-header">
                ...
                <div className="short-score">
                  Long 점수: <strong>{(coin.long_score || 0).toFixed(2)}/100</strong>
                </div>
                <div className="chart-buttons">
                  <button
                    onClick={() => {
